...
# ==========================================
# === IRONWAVES POS - V6.11 (SCHEMA DOCTOR) ===
# ==========================================

VERSION = "v6.11 (Auto-Schema Fix & UnitCost)"
...
def ensure_schema():
    with conn.session as s:
        ...
        # 2. SCHEMA DOCTOR (FIX MISSING COLUMNS AUTOMATICALLY)
        # Missing columns will be added here if they don't exist
        ...
        # SALES FIX
        try: s.execute(text("ALTER TABLE sales ADD COLUMN IF NOT EXISTS customer_card_id TEXT")); s.commit()
        except: pass
        ...
        # TABLES FIX
        try: s.execute(text("ALTER TABLE tables ADD COLUMN IF NOT EXISTS active_customer_id TEXT")); s.commit()
        except: pass

        # RECIPES FIX (CRITICAL)
        ...
        # CUSTOMERS FIX
        try: s.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS created_at TIMESTAMP")); s.commit()
        except: pass
        try: s.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS activated_at TIMESTAMP")); s.commit()
        except: pass
        try: s.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS staff_note TEXT")); s.commit()
        except: pass
